...
function PupilForm() {
  ...
  const handleParentChange = (e, role) => {
    const { name, value } = e.target;
    setFormData({
      ...formData,
      parent: {
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-10 px-4">
      <div className="max-w-3xl mx-auto bg-white p-8 rounded-xl shadow-lg space-y-8 font-sans">
        {/* Header and success message unchanged */}
        <div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <input name="middleName" value={formData.middleName} onChange={handleChange}
                placeholder="Middle Name (optional)" className="border rounded px-3 py-2 w-full text-sm" />
          ...
          {/* Parent Info, Story, Submit Button — unchanged */}
          <div className="space-y-4">
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-700">Who is responsible for the child?</label>
              <select
                name="responsiblePartyType"
                value={formData.responsiblePartyType}
                onChange={(e) => setFormData({ ...formData, responsiblePartyType: e.target.value.toUpperCase() })}
                className="border rounded px-3 py-2 w-full text-sm"
                required>
          ...
          {formData.responsiblePartyType === 'GUARDIAN' ? (
            ...
            <ParentFields formData={formData} setFormData={setFormData} handleParentChange={handleParentChange} />
          )}
